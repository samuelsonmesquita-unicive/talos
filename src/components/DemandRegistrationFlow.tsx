      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-3">
          ...
          <span className="badge-unicive-green self-start sm:self-auto text-[11px]">
            custo = quantidade &times; salário c/ encargos (+50,44%) &times; 6 meses
          </span>
        </div>
              ...
              <div>
                ...
                <span className="text-[11px] text-slate-500 mt-1 block">
                  {activeCargo === 'Mediador'
                    ...
                    : cargaHorariaProf
                    ? `Salário c/ encargos (+50,44%): ${formatCurrency(salarioUnitarioProf)} / mês (Base: ${formatCurrency(salarioBaseProf)})`
              ...
              <div className="text-xs">
                ...
                {cargaHorariaProf && numProf > 0 && detalhesProf && (
                  <span className="block text-[11px] text-slate-500 mt-0.5">
                    Composição unitária: Base {formatCurrency(detalhesProf.salarioBase)} + Trabalhistas (13º/Férias/1/3) {formatCurrency(detalhesProf.subtotalTrabalhista)} + Encargos (INSS 8%/Adicionais 23%) {formatCurrency(detalhesProf.subtotalEncargos)}
                  </span>
                )}
              </div>
          ...
          >
              ...
              <div>
                ...
                <span className="text-[11px] text-slate-500 mt-1 block">
                  {!isProfessorSalvoNesteSemestre
                    ...
                    ? 'Quantidade é zero: carga horária facultativa/dispensada (custo R$ 0,00).'
                    : cargaHorariaMed
                    ? `Salário c/ encargos (+50,44%): ${formatCurrency(salarioUnitarioMed)} / mês (Base: ${formatCurrency(salarioBaseMed)})`
                    : 'Selecione uma das cargas horárias (10h, 20h ou 40h) para liberar o botão de salvar.'}
                </span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-200/80 text-xs">
              ...
              {isProfessorSalvoNesteSemestre && cargaHorariaMed && numMed > 0 && detalhesMed && (
                <span className="block text-[11px] text-slate-500 mt-0.5">
                  Composição unitária: Base {formatCurrency(detalhesMed.salarioBase)} + Trabalhistas (13º/Férias/1/3) {formatCurrency(detalhesMed.subtotalTrabalhista)} + Encargos (INSS 8%/Adicionais 23%) {formatCurrency(detalhesMed.subtotalEncargos)}
                </span>
              )}
            </div>
          </div>
